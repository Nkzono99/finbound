...
contains
    ...
    pure function cylinderXYZ_is_overlap(self, sdoms) result(is_overlap)
        class(t_CylinderXYZ), intent(in) :: self
        ...
        if (self%origin(axis1) + self%radius < sdoms(1, axis1) - 1 &
            .or. sdoms(2, axis1) < self%origin(axis1) - self%radius) then
            is_overlap = .false.
            return
        end if

        if (self%origin(axis2) + self%radius < sdoms(1, axis2) - 1 &
            .or. sdoms(2, axis2) < self%origin(axis2) - self%radius) then
            is_overlap = .false.
            return
        end if
